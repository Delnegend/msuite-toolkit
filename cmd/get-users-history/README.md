# M-Suite User History Data Extraction CLI Usage

## Description
This tool extracts a comprehensive history for each user, including:
- MFA status and configuration.
- A list of all devices associated with the user.
- The last known IP address for each device.
- A history of failed login attempts.
...
- `-output`: path to output CSV file (default: `users_history.csv`)
- `-h` or `-help`: show help
