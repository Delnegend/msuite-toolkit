...
- Sau khi điền `config.toml`, chạy `./set-inactive-users.exe -input users.txt`. Công cụ sẽ gọi Admin Portal để khóa từng user và ghi kết quả vào file CSV đầu ra (xem `-output`).
- File đầu ra có dạng:
