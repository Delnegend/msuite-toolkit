# Công cụ trích xuất dữ liệu MFA, Thiết bị & IP cuối của Người dùng M-Suite
...
```
./users-mfa-devices-ip.exe
```

## Tham số
- `-config`: đường dẫn tới file config (mặc định: `./config.toml`)
- `-output`: đường dẫn file CSV đầu ra (mặc định: `users_mfa_devices_ip.csv`)
