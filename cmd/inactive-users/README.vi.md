...
- `-output`: đường dẫn file CSV đầu ra (mặc định: chạy `inactive-users.exe -help` để xem tên mặc định). File này là nhật ký kết quả: CSV phân tách bằng `|` với header `UserID|Result`, giá trị `Result` là `OK` hoặc thông báo lỗi.
