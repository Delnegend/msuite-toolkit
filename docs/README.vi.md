# Công cụ trích xuất dữ liệu M-Suite
...
- `-output`: đường dẫn file CSV đầu ra (mặc định: chạy `main.exe -help` để xem tên mặc định)
...
**Ghi chú khi chạy**
- Sau khi điền `config.toml`, chạy `./main.exe`. File đầu ra mặc định sẽ xuất hiện cùng thư mục với công cụ.
